...
public class ConcertController {
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "성공적으로 콘서트 목록을 반환", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ConcertScheduleDto.class))),
	...
	public ResponseEntity<List<ConcertScheduleDto>> getAvailableConcertDates(
		@Parameter(description = "조회 시작 날짜 및 종료 날짜를 포함한 콘서트 스케줄 DTO", required = true) @RequestBody ConcertScheduleDto concertScheduleDto) {

		// 날짜 없이 전체 예약 가능한 콘서트 조회
		return ResponseEntity.ok(
			concertUseCase.getAvailableConcertDates(concertScheduleDto.getStartDt(), concertScheduleDto.getEndDt()));
	}

	@Operation(summary = "특정 콘서트 조회", description = "콘서트 ID를 기반으로 특정 콘서트의 상세 정보를 조회합니다.")
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "성공적으로 콘서트 정보를 반환", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ConcertDto.class))),
		@ApiResponse(responseCode = "400", description = "잘못된 콘서트 ID", content = @Content),
		@ApiResponse(responseCode = "500", description = "서버 오류", content = @Content)})
