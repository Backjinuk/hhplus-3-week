...
public class TokenRepositoryImplTest {
	...
	/**
	 * 테스트 데이터 예시 (test-data-token.sql)
	 *
	 * -- Token 더미 데이터
	 * INSERT INTO token (id, user_id, token, issued_at, expires_at)
	 * VALUES
	...
	@Test
	@DisplayName("토큰 만료 - 정상적으로 expiresAt이 현재 시간으로 변경됨")
	void 토큰만료_정상적으로expiresAt이현재시간으로변경됨() {
		// Given
		String tokenStr = "token123";

		// When
		tokenRepositoryImpl.expireToken(tokenStr);

		// Then
		Optional<Token> tokenOpt = tokenRepositoryImpl.authenticateToken(tokenStr);
		assertThat(tokenOpt).isNotPresent(); // expiresAt이 현재 시간으로 변경되어 인증 실패
	}
